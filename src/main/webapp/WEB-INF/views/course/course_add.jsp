...
<section id="blg_sec">
	<div class="container">
		<div class="row">
			<div class="title_sec">
				<br>
				<hr><h1>${mem_id}님 코스를 작성해주세요</h1><hr>
				<input type="button" class="btn btn-primary" value="목록으로" onclick="history.back()" 
				style="float:right">
			</div>
			<!-- 폼 -->
			<form role="form" method="post" action="/course/add">
					
					<input type="hidden" name="mem_id" id="mem_id" value="${sessionScope.login.mem_id}" />
					
					<div class="form-group">
						<label for="cour_title">Title</label>
						<input type="text" name="cour_title" id="cour_title"class="form-control" placeholder="코스 타이틀 입력">
					</div>
					<div class="form-group">
						<label for="cour_days">코스기간</label><br>
						<input type="radio" name="cour_days" id="cour_days" value="1">1일코스
						<input type="radio" name="cour_days" id="cour_days" value="2">2일코스
						<input type="radio" name="cour_days" id="cour_days" value="3">3일코스
						<input type="radio" name="cour_days" id="cour_days" value="4">그외
					</div>
					<div class="form-group">
						<label for="cour_hashtag">해쉬태그</label>
						<input type="text" name="cour_hashtag" id="cour_hashtag" class="form-control" placeholder="해쉬 태그 입력">
					</div>
							...
							<tr>
								...
								<td>
									...
									<table class="table" id="cartList">
									    <tbody>
									    <c:forEach items="${list_plc}" var="placeVO">
									      <tr>
									      	<td class="category">${placeVO.plc_category}</td>
									        <td><img src="/displayFile?fileName=${placeVO.plc_img}" style="width: 60px; height: 60px" class="img-circle" alt=""/></td>
...
<tr>
	<input type="hidden" name="cour_plc_no" id="cour_plc_no" value={{cour_plc_no}}></input>
	<td>{{plc_category}}</td>
  	<td><img src="/displayFile?fileName={{plc_img}}" style="width: 60px; height: 60px" alt=""/></td>
 	<td>{{plc_name}}</td>
