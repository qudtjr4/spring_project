...
<!-- location 상세페이지 start blog Section -->
<section>
	<div class="container">
		<div class="row">
			<div class="title_sec">
...
<!-- 지역 게시판 -->
<%-- <form role="form" method="" action="/admin/locationInput">
<input type="hidden" name="mem_id" value="${sessionScope.login.mem_id}"/> --%>
					...
					<thead>
						<c:forEach items="${list}" var="LocationVO">
							<tr>
								...
								<td>
									<div class="chkclass">
										<input type="checkbox" name="loc_no" id="chk"
											value="${LocationVO.loc_no }" /> ${LocationVO.loc_no }
									</div>
								</td>
								<td><img style="height:150px; width: 150px;" src="/displayFile?fileName=${LocationVO.loc_img}">
								</td>

								<td class="td_sytle"><a
									href="/admin/locationModify?loc_no=${LocationVO.loc_no}">
										${LocationVO.loc_name} </a></td>
							</tr>
						</c:forEach>
				</table>
		...
		<button type="button" class="btn btn-primary" id="btnLocRegist" onclick="/admin/locationInput"> 지역 등록 </button>		
<!-- </form> -->
...
// 지역 등록 버튼
	$('#btnLocRegist').click(function() {
		location.href = '/admin/locationInput';
	});
	...
	$('#btnDelete').click(function(){
		...
		if($(".chkclass :checked").size() < 1) {
			...
			$(".chkclass :checked").each(function() {
				if(param == "") {
					param = "loc_no=" + $(this).parent().children("#chk").val();
					console.log(param);
					console.log("param");
				} else {
					param = param + "&loc_no=" + $(this).parent().children("#chk").val();
					console.log(param);
				}
			});
		}
...
// 		location.href="/location/locationManagement";
